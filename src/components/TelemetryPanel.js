...
import { Activity, Cpu, Zap, Clock, Database, Terminal, Search, Calculator, Brain, Code, CheckCircle2, Loader2, } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-3 border-b border-cyan-500/10">
          ...
          <div className="space-y-2.5">
            <Metric icon={<Cpu size={12}/>} label="CPU Load" value={status ? `${status.cpu.toFixed(0)}%` : '—'} progress={status?.cpu ?? 0} color="cyan"/>
            <Metric icon={<Database size={12}/>} label="Memory" value={status ? `${status.memory.toFixed(0)}%` : '—'} progress={status?.memory ?? 0} color="emerald"/>
            <Metric icon={<Zap size={12}/>} label="Latency" value={status ? `${status.latency}ms` : '—'} progress={status ? Math.min(status.latency / 2, 100) : 0} color="amber"/>
            <Metric icon={<Clock size={12}/>} label="Uptime" value={status?.uptime ?? '—'} progress={75} color="cyan"/>
          </div>
        </div>
        ...
        <div className="px-4 py-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            {sending && (<span className="ml-auto text-[9px] text-cyan-400 font-mono animate-pulse">LIVE</span>)}
          </div>

          {steps.length === 0 && !sending && (<div className="text-center py-6">
          ...
          <AnimatePresence>
            {steps.map((step, i) => (<motion.div key={step.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }} className="mb-2">
                <TraceStep step={step} index={i}/>
              </motion.div>))}
          </AnimatePresence>

          {sending && steps.length === 0 && (<div className="flex items-center gap-2 text-[10px] text-cyan-400 font-mono">
...
function Metric({ icon, label, value, progress, color, }) {
    ...
    return (<div>
      ...
      <div className="h-1 bg-ink-800/60 rounded-full overflow-hidden">
        <motion.div className={`h-full ${c.bar} rounded-full`} initial={{ width: 0 }} animate={{ width: `${Math.min(progress, 100)}%` }} transition={{ duration: 0.5 }}/>
      </div>
    </div>);
}
function TraceStep({ step, index }) {
    ...
    return (<div className={`flex items-start gap-2 ${isResult ? 'pl-5' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {!isResult && step.status === 'running' && (<Loader2 size={10} className="animate-spin text-cyan-400"/>)}
        </div>
        {isResult && step.result && (<p className="text-[10px] text-ink-400 mt-0.5 leading-relaxed font-mono">
            {step.result.slice(0, 120)}
            {step.result.length > 120 ? '...' : ''}
          </p>)}
        {!isResult && step.args && (<p className="text-[9px] text-ink-600 mt-0.5 font-mono">
