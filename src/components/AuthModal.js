...
export default function AuthModal({ isOpen, onClose, onSuccess, dismissible = true }) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md">
      <div className="relative max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-cyan-500/20 bg-slate-900 shadow-2xl shadow-cyan-500/10">
        ...
        {dismissible && (<button onClick={onClose} className="absolute right-4 top-4 rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-slate-800 hover:text-white" aria-label="Close authentication">
            <X size={18}/>
          </button>)}
        ...
        <div className="p-7 sm:p-8">
          ...
          <div className="mb-5 grid grid-cols-3 gap-1 rounded-xl border border-slate-800 bg-slate-950 p-1">
            {['signin', 'signup', 'phone'].map((item) => (<button key={item} type="button" onClick={() => switchMode(item)} className={`rounded-lg py-2 text-[11px] font-medium transition-colors ${mode === item ? 'bg-cyan-500/15 text-cyan-300' : 'text-slate-500 hover:text-slate-200'}`}>
          ...
          {mode === 'phone' ? (<form onSubmit={handlePhoneSubmit} className="space-y-4">
              ...
              {otpSent && (<input inputMode="numeric" autoComplete="one-time-code" value={otp} onChange={(event) => setOtp(event.target.value)} placeholder="Verification code" required className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 outline-none focus:border-cyan-500"/>)}
            ...
            </form>) : (<form onSubmit={handleEmailSubmit} className="space-y-4">
              ...
              {mode === 'signup' && (<input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirm password" required className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 outline-none focus:border-cyan-500"/>)}
              <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500/15 px-4 py-2.5 text-sm font-medium text-cyan-300 transition-colors hover:bg-cyan-500/25 disabled:opacity-50">
                {loading ? <Loader2 size={16} className="animate-spin"/> : <ArrowRight size={16}/>}
