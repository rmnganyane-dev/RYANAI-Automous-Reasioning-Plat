...
export default function AboutModal({ open, onClose }) {
    return (<Modal open={open} onClose={onClose} title="About RyanAI" icon={<Info size={16}/>} maxWidth="max-w-2xl">
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="space-y-2">
            <PortfolioItem icon={<Cpu size={14}/>} name="Project Ryan (RyanAI)" desc="Autonomous AI reasoning agent — LangGraph state persistence, WebSocket backends"/>
            <PortfolioItem icon={<Shield size={14}/>} name="Generation X Firewall" desc="High-performance network security engine in Go using WinDivert for kernel-level packet inspection"/>
            <PortfolioItem icon={<Shield size={14}/>} name="VulaSovereign" desc="Independent decentralized identity and cryptographic platform for self-sovereign identity"/>
            <PortfolioItem icon={<Code size={14}/>} name="Nexus-UNMS" desc="UJ WiFi authentication system — kernel-level packet filtering for high-density environments"/>
            <PortfolioItem icon={<Code size={14}/>} name="ITSM Agent System" desc="Bespoke IT service management platform for focused operational intelligence"/>
          </div>
        </div>
...
function PortfolioItem({ icon, name, desc }) {
    return (<div className="flex items-start gap-3 bg-ink-800/30 rounded-lg p-3 border border-cyan-500/10">
...
function ArchItem({ label, value }) {
    return (<div className="bg-ink-800/30 rounded-lg p-2.5 border border-cyan-500/10">
