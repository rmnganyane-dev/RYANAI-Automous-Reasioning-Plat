...
export async function loadVector(id: string): Promise<VectorEntry | null> {
  ...
  try {
    ...
    return JSON.parse(new TextDecoder().decode(plaintext)) as VectorEntry;
  } finally {
    database.close();
  }
}
